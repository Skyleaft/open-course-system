...
public sealed partial class WordQuestionBankService : IWordQuestionBankService
{
        ...
        return await Task.Run(() =>
        {
            ...
            foreach (var paragraph in paragraphs)
            {
                ...
                else if (isExplicitOption)
                {
                    ...
                    var optionText = optMatch.Success ? optMatch.Groups[3].Value.Trim() : text;
                    ...
                    {
                        Letter = letter,
                        Text = string.IsNullOrWhiteSpace(optionText) ? text : optionText,
                ...
                else if (currentBuilder != null)
                {
                    ...
                    {
                        // Check if it's an inline answer format like A. 10 or similar
    ...
    public byte[] GenerateTemplateDocx()
    {
        ...
        using (var doc = WordprocessingDocument.Create(memoryStream, WordprocessingDocumentType.Document, true))
        {
            ...
            body.Append(CreateParagraph("Guidelines: Use standard numbering for questions (1., 2.) and choices (A., B., C., D.). Specify correct answers using 'Answer: A' or by putting an asterisk '*' before the correct choice like '*A.'. Points and explanations are optional.", false, "64748B"));
            ...
            body.Append(CreateParagraph("Points: 3", false, "475569"));
            ...
            body.Append(CreateHeadingParagraph("4. In relational databases, a foreign key can only reference the primary key of another table, never a candidate unique key.", 24, "0F172A", true));
            ...
            body.Append(CreateHeadingParagraph("5. Explain the concept of ACID properties in Database Management Systems and briefly describe each property.", 24, "0F172A", true));
    ...
    private sealed class ParsedQuestionBuilder
    {
        ...
        public ParsedQuestionItem Build(int questionIndex)
        {
            ...
            else
            {
                ...
                // Fallback: If no option was marked correct, default first option as correct so it is valid
                if (correctCount == 0 && parsedOptions.Count > 0)
                {
